...
class Job(models.Model):
    ...
    infile = models.CharField(max_length=256, blank=True, default="INFILE")
    ...
    # Determines what edit view is shown.
    # Useful if the app has different editing mechanisms
    ...
    def generate_batch(self):
        content = ""
        content += '#PBS -l walltime=%d:%d:00\n' % (self.maxwalltime.hour, self.maxwalltime.minute)
        content += '#PBS -N TOUGHwimp:%s\n' % self.jobname
        content += '#PBS -q ' + self.queue + '\n'
        ...
        content += "aprun -n %d %s %s \n" %(self.numprocs, self.executable, self.infile)
    ...
    def get_comp_settings_form(self):
        return CompSettingsForm(job=self, initial={"queue": self.queue,
...
class CompSettingsForm(forms.Form):
    executable = forms.ChoiceField(choices=settings.EXE_CHOICES)
    queue = forms.ChoiceField(choices=settings.QUEUE_CHOICES)
    num_procs = forms.IntegerField()
    max_walltime = TimeSelectorField()
    email_notifications = forms.MultipleChoiceField(choices=settings.EMAIL_CHOICES, widget=forms.CheckboxSelectMultiple(), required=False)

    def __init__(self, job=None, *args, **kwargs):
        super(CompSettingsForm, self).__init__(*args, **kwargs)
        if(job!=None and job.edit_type != 1):
...
class ImportBlockForm(forms.Form):
    ...
    def __init__(self, user, job_id, *args, **kwargs):
        ...
        self.fields['jobchoice'].queryset = user.job_set.filter(edit_type = 1).exclude(pk = job_id).exclude(exists = False)
