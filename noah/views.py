...
def rebuild_job(request, job_id):
    ...
    return redirect("noah.views.job_edit", job_id=j.pk)
...
def batch_move(request):
    if request.method == "POST":
        ...
        if request.is_ajax():
            return HttpResponse(simplejson.dumps({"success": True, "redirect": reverse("noah.views.jobs")}), content_type="application/json")
        return redirect('noah.views.jobs')
    else:
...
def ajax_submit(request, job_id):
    ...
    return redirect('noah.views.job_view', job_id)
...
def file_upload(request, job_id, file_type):
    ...
    if j.edit_type == 1 and file_type == 'infile':
        ...
        return HttpResponse(simplejson.dumps({"success": True, "redirect": reverse("noah.views.job_edit", kwargs={"job_id": j.pk})}), content_type="application/json")
    else:
        if file_type == "file":
    ...
    return redirect("noah.views.job_edit", j.pk)
...
def create_project(request):
    ...
    return render_to_response("popup_form_base.html", {"form": form, "form_action": reverse("noah.views.create_project"), "form_title": "Create Project", "msg_success":"Project successfully created!"}, context_instance=RequestContext(request))
...
def edit_project(request, project_id):
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            if request.is_ajax():
                return HttpResponse(simplejson.dumps({"success": True, "redirect": reverse("noah.views.project_view", kwargs={"project_id": project_id})}), content_type="application/json")
            return redirect("noah.views.jobs")
    else:
        form = ProjectForm(user=request.user, instance = p, initial = {"jobs":p.job_set.all()})
    return render_to_response("popup_form_base.html", {"form": form, "form_action": reverse("noah.views.edit_project", kwargs={"project_id": project_id}), "form_title": "Edit Project"}, context_instance=RequestContext(request))
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            if Job.objects.get(pk=job.pk).jobdir != form.cleaned_data['jobdir']:
                ...
                form.save()
            if request.is_ajax():
                return HttpResponse(simplejson.dumps({"success": True, "redirect":reverse("noah.views.job_edit", kwargs={"job_id": job.pk}),"info": {"name": job.jobname, "project": job.project.name if job.project else ""}}), content_type="application/json")
            return redirect("noah.views.job_edit", job_id=job.pk)
    else:
        form = InfoEditForm(instance=job, job=job)
    return render_to_response("popup_form_base.html", {"form_title": "Edit Job Information", "form_action": reverse("noah.views.info_edit", kwargs={"job_id": job.pk}), "form": form}, context_instance=RequestContext(request))
